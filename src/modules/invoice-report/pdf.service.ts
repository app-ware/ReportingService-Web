...
@Injectable()
export class InvoiceReportPdfService implements OnModuleInit, OnModuleDestroy {
  private browser: Browser;

  constructor(private readonly logger: PinoLogger) {
    this.logger.setContext(InvoiceReportPdfService.name);
  }

  async onModuleInit() {
    this.logger.info('Initializing Invoice Report PDF Service...');
    this.browser = await chromium.launch();
    this.logger.info('Invoice Report PDF Service initialized successfully');
  }
  ...
  private buildFooterOptions(data: InvoiceReportTemplateData): { footerTemplate: string; marginBottom: string } {
    ...
    <span style="flex: 1; text-align: center;">Invoice Report</span>
  ...
  async generatePdfFromTemplate(data: InvoiceReportTemplateData): Promise<Buffer> {
    this.logger.info('Starting PDF generation...');

    const templatePath = path.join(__dirname, '..', '..', 'templates', 'invoice-report', 'invoice-report.hbs');
